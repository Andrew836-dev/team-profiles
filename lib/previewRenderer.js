...
const preview = allEmployees => {
    ...
    const printList = [
        employees.map(employee => employee.getRole()).map(name => renderWordSection(name)),
        employees.map(employee => employee.getName()).map(name => renderWordSection(name)),
        employees.map(employee => employee.getId()).map(name => renderWordSection(name)),
        employees.map(employee => employee.getEmail()).map(name => renderWordSection(name)),
    ]
    const specialList = [
        employees.map(employee => getSpecialTitle(employee)),
        employees.map(employee => getSpecialData(employee)).map(name => renderWordSection(name))
    ]
